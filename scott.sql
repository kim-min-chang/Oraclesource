...
-- 각 부서별 평균 급여, 최대급여, 최소 급여, 사원수를 조회
-- 부서번호, 부서명, 평균 급여(AVG_SAL), 최대급여, 최소급여, 사원수
...
--모든 부서 정보와 사원 정보를 조회
-- 부서번호, 부서명, 사원 번호, 사원명, 직무(JOB),급여
SELECT D.DEPTNO, D.DNAME , E.EMPNO ,E.ENAME ,E.JOB ,E.SAL 
FROM EMP E RIGHT OUTER JOIN DEPT d ON E.DEPTNO = D.DEPTNO 
GROUP BY D.DEPTNO ,D.DNAME , E.EMPNO ,E.ENAME ,E.JOB ,E.SAL 
ORDER BY D.DEPTNO, E.EMPNO ;

-- 서브쿼리 : 쿼리문 안에 또 다른 쿼리문이 포함(SELECT,UPDATE,DELETE,INSERT)
--SELECT 
--FROM  
...
-- 20번 부서에 근무하는 사원 중 전체 사원의 평균 급여보다 높은 급여를 받은 사원조회 + 부서명, 부서위치
...
-- ALL : 메인 쿼리의 조건식을 서브 쿼리의 결과 모드가 만족하면 TRUE
...
-- 30번 부서의 최대 급여 보다 적은 사원 조회하는 결과와 같아짐
-- 다중행 서브쿼리로 할때
SELECT *
FROM EMP e 
...
-- 30번 부서의 최저 급여 보다 많ㅇ느 사원 조회하는 결과와 같아짐
-- 다중행 서브쿼리로 할때
SELECT *
FROM EMP e 
WHERE E.SAL > ANY (SELECT E2.SAL FROM EMP e2 WHERE E2.DEPTNO = 30)
...
SELECT *
FROM EMP e 
WHERE EXISTS(SELECT D.DNAME FROM DEPT d WHERE DEPTNO = 10);


SELECT *
FROM EMP e 
WHERE EXISTS(SELECT D.DNAME FROM DEPT d WHERE DEPTNO = 50);
...
FROM (SELECT *FROM EMP WHERE DEPTNO=10) E10 , (SELECT * FROM DEPT d) D
...
SELECT * FROM EMP e WHERE EMPNO = 7369;
...
--EMP 테이블과 SALGRADE 조인 후 급여등급이 1인 사원 들만 EMP_TEMP 에 삽입(서브쿼리)
--데이터가 추가되는 테이블 열 개수와 서브쿼리 열 개수 와 자료형 일치
...
--EMP_TEMP 의 사원들 중에서 급여가 2500 이하인 사원들의 추가수당을 50으로 변경

UPDATE EMP_TEMP 
SET COMM = 50
WHERE SAL <= 2500;

-- DEPT테이블의 DEPTNO=40 부서의 DNAME,LOC 를 추룰해서 DETP_TEMP2 의 DNAMEM,LOC 수정
...
-- 서브쿼리
-- 급여등급이 3등급이고 , 부서번호가 30번인 사원 삭제

DELETE FROM EMP_TEMP2 
WHERE 
EMPNO IN (SELECT E.EMPNO 
FROM EMP_TEMP2 E JOIN SALGRADE s ON E.SAL BETWEEN S.LOSAL AND S.HISAL 
WHERE S.GRADE = 3 AND E.DEPTNO = 30);

-- 테이블 복사
-- EMP,DEPT,SALGRADE 테이블 복사
-- EXAM_EMP,EXAM_DEPT , EXAM_SLAGRADE 로 생성
CREATE TABLE EXAM_EMP AS SELECT * FROM EMP;
CREATE TABLE EXAM_DEPT AS SELECT * FROM DEPT;
CREATE TABLE EXAM_SALGRADE AS SELECT * FROM SALGRADE;

-- INSERT
-- EXAM_DEPT 테이블에 50,60,70,80 부서등록
...
-- UPDATE
-- EXAM_EMP 에 속한 사원 중 50번 부서에서 근무하는 사원들의 평균 급여보다 많은 급여를 받고 있는 사원들을 70번 부서로 변경
UPDATE EXAM_EMP 
SET DEPTNO = 70
WHERE SAL >= (SELECT MAX(SAL) FROM EXAM_EMP ee WHERE DEPTNO=50);

--EXAM_EMP 에 속한 사원 중 60번 부서의 사원 중에서 입사일이 가장 빠른 사원보다 늦게 입사한 사원의 급여를 10%인상후 60번 부서로 변경
UPDATE EXAM_EMP 
SET SAL = SAL*1.1,DEPTNO =80
WHERE HIREDATE > (SELECT MIN(HIREDATE) FROM EXAM_EMP ee WHERE DEPTNO= 60);
-- DELETE
--EXAM_EMP 에 속한 사원중 급여 등급이 5인 사원 삭제

DELETE FROM EXAM_EMP 
WHERE 
EMPNO IN (
SELECT EMPNO
FROM EXAM_EMP e JOIN EXAM_SALGRADE s ON E.SAL BETWEEN S.LOSAL AND S.HISAL AND S.GRADE = 5);
