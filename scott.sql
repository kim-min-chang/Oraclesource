...
--MAXVALUE 99999999999999999999999999999(시퀀스에서 생성할 최대값)
--NOCYCLE (최대값 도달 후 다시 시작값부터 시작할 것인지 여부)
...
--시퀀스 DEPT_SEQ.NEXTVAL exceeds MAXVALUE은 사례로 될 수 없습니다
-- DEPT_SEQ.NEXTVAL EXCEEDS MAXVALUE 위반
...
--CREATE SYNONYM 동의어 이름 FOR 테이블 명
...
-- 제약 조건 : 특정 열에 지정
--1) NOT NULL : 지정된 열에 NULL 을 허용하지 않음
--2) UNIQUE : 지정된 열이 유일한 값을 가져야 함 
--3) PRIMARY KEY : 지정한 열이 유일한 값이면서 NULL 허용하지 않음(테이블당 하나만 지정가능)
--4) FOREIGN KEY : 다른 테이블의 열을 참조하여 존재하는 값만 입력
--5) CHECK : 설정한 조건식을 만족하는 데이터만 입력가능
...
INSERT INTO TABLE_NOTNULL VALUES('TEST_ID_01','TEST_PWD_01','010-1234-5678');
...
-- 중복 여부만 확인(NULL 을 확인하지는 않음)
INSERT INTO TABLE_UNIQUE VALUES (NULL,'TEST_PW_01','010-1234-5678');

CREATE TABLE TABLE_UNIQUE2(
 	LOGIN_ID VARCHAR2(20) CONSTRAINT TBLUNQ2_LNG_UNQ UNIQUE,
 	LOGIN_PWD VARCHAR2(20) CONSTRAINT TBLUNQ_LGNPW_UNQ UNIQUE,
 	TEL VARCHAR2(20)
 );
...
-- 3) PRIMARY KET(PK - 기본키) : NOT NULL + UNIQUE
-- 기본키 : 주민등록번호, 사원번호, 아이디
-- 하나의 컬럼만 지정 가능
...
-- DEPT 테이블의 DEPT 의 값을 기반으로 EMP 테이블의 DEPTNO 값 삽입
INSERT INTO EMP 
...
DELETE FROM EMP_FK WHERE DEPTNO = 50;
-- 외래키 제약 조건이 걸린 테이블 규칙
-- 1) 삽입 시 부모 부터 데이터 삽입
-- 2) 삭제 시 자식 데이터 부터 삭제  

-- ON DELETE CASCADE : 부모 데이터 삭제 시 참조하고 있는 자식 데이터 같이 삭제
-- ON DELETE SET NULL : 부모 데이터 삭제시 참조하고 있는 자식 데이터의 값을 NULL로 변경
CREATE TABLE EMP_FK2(
	...
	SAL NUMBER(7,2),
	COMM NUMBER(7,2),
	DEPTNO NUMBER(2,0) CONSTRAINT EMPFK_DEPTNO_FK REFERENCES DEPT_FK(DEPTNO) ON DELETE CASCADE
);
...
CREATE TABLE EMP_FK3(
	...
	SAL NUMBER(7,2),
	COMM NUMBER(7,2),
	DEPTNO NUMBER(2,0) CONSTRAINT EMPFK_DEPTNO_FK REFERENCES DEPT_FK(DEPTNO) ON DELETE SET NULL 
);
...
INSERT INTO BABLE_CHECK VALUES('TEST_ID_01','12',NULL);
...
INSERT INTO TABLE_DEFALUT VALUES ('TEST_ID_01',NULL,NULL);
INSERT INTO TABLE_DEFALUT(LOGIN_ID,TEL) VALUES ('TEST_ID_01',NULL);
SELECT * FROM TABLE_DEFALUT;

-- BOARD 테이블 정의
-- BNO(숫자), NAME,PASSWORD , TITLE,CONTENT,READCNT(숫자 - DEFAULT 0), REGDATE(날짜 - DEFAULT 현재 시스탬 날자)
-- 기본키 : BNO
-- NOT NULL : NAME,PASSWORD , TITLE , CONTENT

-- 시퀸스 생성 : 1씩 증가 BOARD_SEQ
